...
class App (rapidsms.app.App):
    ...
    def send_reminders(self,message):
    	""" see expired reporters and fetch supervisor in their locations and send notifications"""
    	#callback="10:21"
    	#remind_time=str(strftime("%H:%M", localtime()))
    	#message=None
    	expired_reporters=Reminder.get_expired_reporters(datetime.today())
    	reminder=ReminderType.objects.get(pk=6)
    	sup_group=ReporterGroup.objects.get(title='Supervisor')
    	
    	for reporter in expired_reporters:
    		location=reporter.location
    		sups=Reporter.objects.filter(groups=sup_group, location=location).order_by("pk")
    		for sup in sups:
    			conn=sup.connection()
    			lang=sup.language
    			text=reminder.message_kw
    			if lang=='en':
    				text=reminder.message_en
    			elif lang=='fr':
    				text=reminder.message_fr
    			sentmsg=_(text % (reporter.alias,reporter.connection().identity))
    			
    			#msg=OutgoingMessage(conn.identity,sentmsg)
    			#if callback==remind_time:
    			#message.forward(conn.identity,sentmsg)
